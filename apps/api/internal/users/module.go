...
func (m *UsersModule) RegisterRoutes(router chi.Router, deps platform.Dependencies) {
	...
	socialVerifier := NewSocialVerifier(deps.Config.GoogleClientID, deps.Config.FacebookAppID)
	tenantSvc := tenants.NewService(tenants.NewRepository(deps.DB), deps.Redis)
	svc := NewService(repo, tokenManager, tenantSvc, socialVerifier)
	...
	router.Get("/api/v1/auth/oauth/google/start", handler.googleOAuthStart)
	router.Get("/api/v1/auth/oauth/google/callback", handler.googleOAuthCallback)
	router.Get("/api/v1/auth/oauth/facebook/start", handler.facebookOAuthStart)
	router.Get("/api/v1/auth/oauth/facebook/callback", handler.facebookOAuthCallback)
